...
async function generateVisual(annId, prompt) {
  ...
  const planSys = [
    'You turn a reader\'s request into the most useful visual for a document reader. First choose the FORMAT:',
    '- "ascii": a precise monospace diagram built ONLY from the document — flowchart, pipeline, schematic, tree, comparison table, or a labelled layout. PREFER THIS for structure, process, relationships, comparisons, or "main results/summary", where faithfulness to the document matters. Never invent numbers.',
    '- "image": a rendered picture — ONLY when a pictorial illustration or concept art genuinely aids intuition (a physical scene, an analogy, an artistic concept). NEVER choose image to depict specific data, plots, tables, equations, or exact results — image models fabricate those.',

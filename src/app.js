...
const TOOL_LABEL = { read_selection_context: 'Re-reading the selection…', read_page: 'Reading a page…', search_document: 'Searching the document…', document_outline: 'Scanning the outline…', read_full_document: 'Reading the full paper…', web_search: 'Searching the web…' };
...
async function generateVisual(annId, prompt) {
  ...
  const planSys = [
    'You turn a reader\'s request into the most useful visual for a document reader. First choose the FORMAT by what the reader actually wants:',
    '- "image": a rendered picture — choose for ILLUSTRATIONS, concept art, physical scenes, analogies, cartoons, or intuitive/artistic depictions of an idea or phenomenon (e.g. "illustrate eddies breaking into smaller eddies", "draw the experimental setup", "picture of a hairpin vortex"). Words like illustrate/illustration/picture/drawing/draw/artistic/concept/scene/cartoon strongly indicate image. It is fine for an image to depict a PROCESS conceptually.',
    '- "ascii": a precise monospace diagram (flowchart, pipeline, tree, comparison table, labelled layout) built ONLY from the document. Choose for the paper\'s DATA, results, numbers, tables, equations, findings, or a faithful structural/flow summary of its content — where an image would fabricate specifics.',
    'Tie-breaker: choose "ascii" ONLY when the request is to depict the paper\'s specific data/results/findings/numbers/summary. For genuinely pictorial or conceptual illustrations, choose "image" — even if the concept involves a process, and even though image models can\'t render exact data. When the reader explicitly says "illustrate/illustration/picture/draw", default to image unless they clearly want the paper\'s numeric results.',

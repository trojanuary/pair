...
function templatesPaneHTML() {
  const items = PROMPT_KEYS.map(k => {
    const meta = PROMPT_META[k] || { label: k, desc: '' };
    const overridden = !!(state.settings.prompts && state.settings.prompts[k] && String(state.settings.prompts[k]).trim());
    return `<div class="pt-item">
      <div class="pt-head"><label>${esc(meta.label)}${overridden ? ' <span style="color:var(--violet);font-weight:700">· customized</span>' : ''}</label>
        <button type="button" class="pt-reset" data-reset="${k}">Reset to default</button></div>
      <textarea id="pt_${k}" spellcheck="false">${esc(promptFor(k))}</textarea>
      <div class="hint">${esc(meta.desc)}</div>
    </div>`;
  }).join('');
  return `<div class="hint" style="margin:-2px 0 14px">Make the assistant your own — edit any prompt below. The per-request context (your selection, question, and page) is always added by the app, so restyling the voice here won’t break how answers are built. Changes apply after you press <b>Save</b>. Export a set to reuse it across machines, or import one you’ve saved.</div>
    <div class="pt-actions"><button type="button" class="btn ghost" id="ptExport">Export prompts (JSON)</button><button type="button" class="btn ghost" id="ptImport">Import prompts (JSON)</button><button type="button" class="btn ghost" id="ptResetAll">Reset all to default</button></div>
    ${items}`;
}
...
function exportPrompts(m) {
  try {
    const data = { app: 'Source-Linked AI Reading Workspace', kind: 'prompt-templates', schema: 1, exportedAt: nowISO(), prompts: {} };
...
function importPrompts(m) {
  ...
  inp.onchange = async () => {
    ...
    try {
      ...
      const p = obj && (obj.prompts || obj); let n = 0;
      PROMPT_KEYS.forEach(k => { if (p && typeof p[k] === 'string') { const ta = document.getElementById('pt_' + k); if (ta) { ta.value = p[k]; n++; } } });
...
function openSettings(note) {
  ...
  $$('[data-reset]', m).forEach(b => b.onclick = () => { const ta = $('#pt_' + b.dataset.reset, m); if (ta) ta.value = DEFAULT_PROMPTS[b.dataset.reset]; });
  { const pe = $('#ptExport', m); if (pe) pe.onclick = () => exportPrompts(m); const pi = $('#ptImport', m); if (pi) pi.onclick = () => importPrompts(m); const pr = $('#ptResetAll', m); if (pr) pr.onclick = () => PROMPT_KEYS.forEach(k => { const ta = $('#pt_' + k, m); if (ta) ta.value = DEFAULT_PROMPTS[k]; }); }

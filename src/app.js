...
function pickImageProvider() {
  const a = activeProvider(); if (canImage(a) && keyFor(a)) return a;
  if (keyFor('openai')) return 'openai'; if (keyFor('gemini')) return 'gemini'; return null;
}
async function aiText(provider, { system, user, image }) {
  const key = keyFor(provider); if (!key) throw new Error(`No API key set for ${PROVIDER_LABEL[provider]}.`);
  ...
  if (provider === 'openai') {
    ...
    const r = await fetch('https://api.openai.com/v1/chat/completions', { method: 'POST',
      ...
      body: JSON.stringify({ model: m.openai, messages: [{ role: 'system', content: system }, { role: 'user', content }], max_tokens: 900 }) });
    const j = await r.json(); if (!r.ok) throw new Error(j.error?.message || 'OpenAI error'); return j.choices[0].message.content.trim();
  }
  if (provider === 'anthropic') {
    ...
    const r = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST',
      ...
      body: JSON.stringify({ model: m.anthropic, max_tokens: 900, system, messages: [{ role: 'user', content }] }) });
    const j = await r.json(); if (!r.ok) throw new Error(j.error?.message || 'Anthropic error'); return j.content.map(c => c.text || '').join('').trim();
  }
  if (provider === 'gemini') {
    const parts = image ? [{ text: user }, { inline_data: { mime_type: image.mime, data: image.b64 } }] : [{ text: user }];
    const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${m.gemini}:generateContent?key=${key}`, { method: 'POST',
...
async function aiImage(provider, prompt) {
  const key = keyFor(provider); if (!key) throw new Error(`No API key for ${PROVIDER_LABEL[provider]}.`);
  const m = state.settings.models;
  if (provider === 'openai') {
    const r = await fetch('https://api.openai.com/v1/images/generations', { method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + key },
      body: JSON.stringify({ model: m.openaiImage, prompt, size: '1024x1024', n: 1 }) });
  ...
  if (provider === 'gemini') {
    const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${m.geminiImage}:predict?key=${key}`, { method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ instances: [{ prompt }], parameters: { sampleCount: 1 } }) });
    const j = await r.json(); if (!r.ok) throw new Error(j.error?.message || 'Imagen error');
    const b = j.predictions?.[0]?.bytesBase64Encoded; if (!b) throw new Error('No image returned'); return 'data:image/png;base64,' + b;
  }
  throw new Error(`${PROVIDER_LABEL[provider]} can't generate images. Set an OpenAI or Gemini key.`);
}
...
async function askAI(annId, question, providerOverride) {
  ...
  if (!keyFor(provider)) { openSettings(`Add your ${PROVIDER_LABEL[provider]} API key to ask live questions.`); return; }
...
async function generateVisual(annId, prompt) {
  ...
  if (!ip) { openSettings('Add an OpenAI or Gemini API key to generate visuals.'); return; }
  const ctx = buildContext(a);
...
function errHint(m) {
  if (/failed to fetch|networkerror|load failed/i.test(m)) return 'Network/CORS blocked the API call. Open the downloaded HTML file directly in your browser (live AI is blocked inside embedded previews).';
  return m;
}
...
function openSettings(note) {
  ...
  const m = el(`<div class="modal-mask"><div class="modal">
    ...
    <div class="body">
      ...
      <div class="hint" style="margin:-2px 0 16px">Bring your own API key. Mark one provider as <b>Default</b> for the composer — you can still target any inline with @gpt / @claude / @gemini.</div>
      <div class="field">
      ...
      <div class="hint">Keys are stored only in your browser (localStorage). For live calls, open the downloaded file directly — embedded previews block cross-site API requests.</div>
    </div>
...
async function boot() {
  ...
  if (!keyFor(state.settings.provider) && !keyFor('openai') && !keyFor('anthropic') && !keyFor('gemini'))
    setTimeout(() => toast('Tip: add an API key in Settings to make “Ask AI” and “Generate visual” live.'), 900);
}
